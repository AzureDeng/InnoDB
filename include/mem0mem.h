...
struct mem_block_info_t {
	ulint	magic_n;/* magic number for debugging */  //魔法值，用于调试
#ifdef UNIV_DEBUG
	char	file_name[8];/* file name where the mem heap was created */     //创建内存堆的代码文件名
	ulint	line;	/*!< line number where the mem heap was created */      //创建内存堆的代码文件行
#endif /* UNIV_DEBUG */
	UT_LIST_BASE_NODE_T(mem_block_t) base; /* In the first block in the     //链表信息
			the list this is the base node of the list of blocks;
			in subsequent blocks this is undefined */
	UT_LIST_NODE_T(mem_block_t) list; /* This contains pointers to next     //链表节点
			and prev in the list. The first block allocated
			to the heap is also the first block in this list,
			though it also contains the base node of the list. */
	ulint	len;	/*!< physical length of this block in bytes */          //块长度，字节单位
	ulint	total_size; /*!< physical length in bytes of all blocks
			in the heap. This is defined only in the base
			node and is set to ULINT_UNDEFINED in others. */
	ulint	type;	/*!< type of heap: MEM_HEAP_DYNAMIC, or
			MEM_HEAP_BUF possibly ORed to MEM_HEAP_BTR_SEARCH */
	ulint	free;	/*!< offset in bytes of the first free position for     //空闲相对位置
